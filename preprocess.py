...
def get_data(args):
    ...
    def convert(textfile, batchsize, seqlength, outfile, num_sents, max_sent_l=0,
                shuffle=0, out_file=None):
        ...
        for sent in open(textfile, 'r'):
            ...
            if out_file is not None:
                new_sent = " ".join([indexer.idx2word[idx] for idx in 
                                    sents[sent_id][:sent_lengths[sent_id]]])
                out_text.write(new_sent + "\n")
            sent_id += 1
            if sent_id % 100000 == 0:
